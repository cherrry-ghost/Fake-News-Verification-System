...
# ---------------- MAIN ----------------
print("LIVE FACT CHECKER v5 SMART v2 (Typo + Synonym + Date + Smart Person)")
...
while True:
    ...
    # ---------------- Detect role with fuzzy matching ----------------
    role = None
    for r, keywords in role_keywords.items():
        for kw in keywords:
            match = get_close_matches(kw, words_no_year, n=1, cutoff=0.7)
            if match:
                role = r
                break
        if role:
    ...
    # ---------------- Detect country with fuzzy matching ----------------
    ...
    if not country:
        match = get_close_matches(raw_country, countries.keys(), n=1, cutoff=0.7)
        if match:
            country = countries[match[0]]
        else:
            country = raw_country.capitalize()

    # ---------------- Detect person (smarter) ----------------
    person = None
    if "is" in words_no_year:
        is_index = words_no_year.index("is")
        if is_index + 1 < len(words_no_year):
            person = words_no_year[is_index + 1].capitalize()
    if not person:
        # fallback: first word that is not a role keyword
        role_words = sum(role_keywords.values(), [])
        for w in words_no_year:
            if w not in role_words and w != "of":
                person = w.capitalize()
                break
